...
class MetricsLogger:
    """Accumulate per-step metrics and flush them as mean/std summaries."""

    def __init__(self):
        self.history = defaultdict(lambda: defaultdict(list))
        self.step_buffer = defaultdict(lambda: defaultdict(list))
        ...
        for split, metric_dict in self.history.items():
            for metric, history in metric_dict.items():
                if metric not in metrics:
                    continue
                latest = history[-1]
                grouped[metric][split] = (
                    f"{latest['mean']:.4f}±{latest['std']:.4f}"
                )

        parts = []
        for metric, split_vals in grouped.items():
            joined = ", ".join(
                f"{split}={val}" for split, val in split_vals.items()
            )
    ...
    def ensure_metrics_exist(self, metrics):
        ...
        if requested:
            all_available = {
                metric
                for split in self.history.values()
                for metric in split.keys()
            }
            missing = requested - all_available
            if missing:
                raise ValueError(
                    f"Missing metrics in history: {sorted(missing)}"
                )
